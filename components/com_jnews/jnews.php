...
switch ($action) {
	...
	case ('sendVerjaardag'): // /index.php?option=com_jnews&act=sendVerjaardag
		...
		$err = array();
		//waspas mailinglist
		$db->setQuery("SELECT id FROM #__jnews_lists WHERE list_name LIKE '%waspas%'");
		$waspasListID = $db->loadResult();
		//verjaardag mailing
		$db->setQuery($db->getQuery(true)
			->select("m.id")
			->from("#__jnews_mailings AS m")
			->innerjoin("#__jnews_listmailings AS lm ON lm.mailing_id = m.id")
			->where("lm.list_id = $waspasListID")
			->order("m.issue_nb ASC")
		,0,1);
		$verjaardagMailingID = $db->loadResult();
		if ($waspasListID) {
			//zoek jarigen
			$query = $db->getQuery(true)
				->select("s.*, ls.*")
				->from("#__jnews_subscribers AS s")
				->innerjoin("#__jnews_listssubscribers AS ls ON ls.subscriber_id = s.id")
				->where("s.column1 LIKE '%".$date->format('-m-d')."'")
				->where("ls.list_id = $waspasListID");
			$db->setQuery($query);
			$jarigen = $db->loadObjectList();
			$message = "\n".'Check verjaardagen: '.JHtml::_('date','now','d-m-Y - H:i:s')."\n";
			if (count($jarigen)) {
				$list = null;
				$new = null;
				$mailing = jNews_Mailing::getOneMailing($list, $verjaardagMailingID, '', $new);
				$list = jNews_Lists::getOneList( $waspasListID );
				foreach ($jarigen as $jarige) {
					$message .= 'Jarige: '.$jarige->name.', '.$jarige->email."\n";
					$receiver = jNews_Subscribers::getSubscribersFromId(array($jarige->id));
					$mailerC = new jNews_ProcessMail();
					$sendStatus = $mailerC->send( $mailing, $receiver );
// echo '<pre>'.$waspasListID;
// print_r($sendStatus);
// print_r($mailerC);
// echo '</pre>';
					if (!$sendStatus) {
						$message .= 'Fout bij verzenden '.$jarige->name."\n";
					} else {
						$message .= 'Succesvol verzonden '.$jarige->name."\n";
					}
				}
				$Name = "Verjaardagsmail"; //senders name 
				$email = "info@happycarwash.nl"; //senders e-mail adress 
				$mail_body = $message; //mail body 
				$subject = "Verzendrapport verjaardagsmail"; //subject 
				$header = "From: Verjaardagsmail <info@happycarwash.nl>\r\n"; //optional headerfields 

				$recipient = "info@styleshift.nl"; //recipient 
				mail($recipient, $subject, $mail_body, $header);
				$recipient = "info@happycarwash.nl"; //recipient 
				mail($recipient, $subject, $mail_body, $header);
			} else {
				$message .= 'Geen jarigen gevonden.';
			}
			$logfile = JPATH_ROOT.DS.'logs'.DS.'verjaardagen.txt';
			@file_put_contents($logfile, $message, FILE_APPEND);
		}
/* 			$query = $db->getQuery(true)
				->select("s.*")
				->from("hcw_acajoom_subscribers AS s")
				->innerjoin("hcw_acajoom_queue AS ls ON ls.subscriber_id = s.id")
				->where("ls.list_id = 3");
			$db->setQuery($query);
			$aSubs = $db->loadObjectList();
			$oSubList = new stdClass;
			$oSubList->list_id = $waspasListID;
			$oSubList->subdate = JFactory::getDate()->toUnix();
			$oSubList->unsubdate = 0;
			$oSubList->unsubscribe = 0;
			$oSubList->params = null;
			foreach ($aSubs as $oSub) {
				$oSub->id = null;
				$oSub->column1 = substr($oSub->subscribe_date,0,10);
				$oSub->subscribe_date = JFactory::getDate()->toUnix();
				$db->setQuery("SELECT id,user_id FROM #__jnews_subscribers WHERE email = '$oSub->email'");
				$oExisting = $db->loadObject();
				if (!$oExisting) {
					if(!$db->insertObject('#__jnews_subscribers',$oSub)) {
						echo $db->getErrorMessage();
					}
				} else {
					$oSub->id = $oExisting->id;
					$oSub->user_id = $oExisting->user_id;
					if(!$db->updateObject('#__jnews_subscribers',$oSub,'id')) {
						echo $db->getErrorMessage();
					}
				}
				$oSubList->subscriber_id = $oSub->id;
echo '<pre>';
print_r($oSub);
print_r($oSubList);
echo '</pre>';	
				if(!$db->insertObject('#__jnews_listssubscribers',$oSubList)) {
					echo $db->getErrorMessage();
				}
			}
 */			
